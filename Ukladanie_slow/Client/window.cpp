...
{
    bool ok;
    const QString newUsername = QInputDialog::getText(this, tr("Wybierz nazwe uzytkownika"), tr("Gracz"),
...
{
   QMessageBox::warning(this, tr("Nie mozna dolaczyc"), tr("Gra jest pelna"));
   user->disconnectFromHost();
}
...
void Window::theEnd(const QString &message)
{
   ...
   if (message.left(11) =="Gre wygral")
   {
       ui->startButton->setEnabled(true);
       ui->submitButton->setEnabled(false);
       ui->messageEdit->setEnabled(false);
   }
   if (message.left(17) == "Slowa przeciwnikow: " || message=="Zaden z przeciwnikow nie znalazl innego slowa")
   {
...
void Window::startRound(const QString &message)
{
    ...
    ui->result->setText("Podaj slowo");
    ui->startButton->setEnabled(false);
    ui->submitButton->setEnabled(true);
    ui->messageEdit->setEnabled(true);
    ui->runda->setText(QString::number(round));
