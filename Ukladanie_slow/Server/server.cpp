...
void sendResult(char *answer, char *message, int who)
{
    char temp[5];
    char *ptr = &message[2];
    printf("Gracz %s %d podal slowo %s\n", players[who].login, players[who].fd, ptr);
    int len = static_cast<int>(strlen(message)) - 2;
    if(len < 3)
        strcpy(answer, "2-Slowo jest za krotkie");
    else if(!checkWord(message))
        strcpy(answer, "2-Z podanych liter nie da sie utworzyc tego slowa");
    else if(!checkDictionary(message))
        strcpy(answer, "2-Nie ma takiego slowa");
    else if(checkBase(message))
        strcpy(answer, "2-Ktos juz podal to slowo");
    else
    {
        players[who].points += strlen(message) - 2;
        strcpy(answer, "2-Podales dobre slowo. Brawo!");
...
void endRound()
{
    ...
    printf("Runda %d sie skonczyla\n", game.roundNumber);
    for (int i = 0; i < USERS; i++)
    {
        strcpy(answer, "4-Slowa przeciwnikow ");
        if(players[i].fd != 0)
        {
            ...
            if(players[i].oppCount == 0)
                strcpy(answer, "4-Zaden z graczy nie znalazl innego slowa");
            sendAnswer(answer, players[i].fd);
        }
    }
    deleteData();
...
void countTime()
{
    ...
    while (true)
        if(game.started == 1)
        {
            ...
            {
                strcpy(answer, "2-Pozostalo 10 sekund");
                sendBroadcast(answer);
                warning = 1;
            }
            sleep(1);
                    ...
                    if(game.playersCount < USERS)
                    {
                        ...
                        for (int i = 0; i < USERS; i++)
                        {
                            if(players[i].fd == 0)
                            {
                                ...
                                getLogin(message, i);
                                printf("Nowy gracz o nazwie %s %d dolaczyl do gry\n", players[i].login, players[i].fd);
                        ...
                        {
                            strcpy(answer, "3-wystarczajaca ilosc graczy");
                            sendBroadcast(answer);
                        }
                        if(game.started == 1)
                            sendData(newPlayer);
